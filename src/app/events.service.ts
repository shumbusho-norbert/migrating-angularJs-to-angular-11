...
export class EventsService {
  ...
  searchSessions(searcTerm: string) {
    var term = searcTerm.toLocaleLowerCase();
    var results: ISession[] = [];
    EVENTS.forEach((event: IEvent) => {
      var matchingSessions = event.sessions.filter((session) => session.name.toLocaleLowerCase().indexOf(term) > -1);
      //lets add eventid in the session object array as we dont have it, below we will create session as any type since 
      //the interface ISession does not have EventId field
...
const EVENTS: IEvent[] = [
  {
    ...
    onlineUrl: 'htps://microsoft.co.za',
    sessions: [
      {
        id: 1,
